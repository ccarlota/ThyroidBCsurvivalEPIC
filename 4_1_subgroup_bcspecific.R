#----------------------------------------------#
# Subgroup analyses for BC-specific mortality
#----------------------------------------------#
# Libraries 
library(haven)
library(foreign)
...
# variable for specific death:0 = Alive, 1 = Specific death (BC-related), 2 = Other death
...
set_metas <- set %>% filter(stage2 == "Metastatic") 
set_nonmetas <- set %>% filter(stage2 == "Localised")
...
write.table(final_table, file = paste0(output.dir,"004_subgroup_bc_specific.txt"), 
...
for (i in exposures) {
  ...
  formula <- as.formula(paste("Surv(age_dx, Age_Exit_Vs, Death_specific =='Dead') ~ ", i,
                              "+ Phrt_Bld_c + bmi_oms + Alc_Re_c + year_diagnosis + gradb + 
                               Er_Adj_c + Pr_Adj_c + Her2_Adj_c + l_school2 +
                               strata(COUNTRY,stage3, menop_status_dx)"))
  
  formula_2 <- as.formula(paste("Surv(age_dx, Age_Exit_Vs, Death_specific =='Dead') ~ ", 
...
write.table(final_table_het, file = paste0(output.dir,"004_Subgroup_het_BCspecific.txt"), 
            col.names = T, row.names = T, sep = "\t", quote = F)
